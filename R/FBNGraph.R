...
FBNNetwork.Graph.CreateDynamicNetworkObjects<-function(networkobject,timepoints,dynamicNetworkGraphicObject)
{
  ...
  while(start<=end)
  {
    ...
    if(length(rownames(filterddynamic))>0)
    {
      ...
      net <- network(newedges, vertex.attr=newnodes, matrix.type="edgelist",
  ...
  return(res)
  #visNetwork(newnodes, newedges, width="100%", height="100%", main="Network!")
  # & as.numeric(dynamicNetworkGraphicObject$onset)>=timepoint & timepoint<=as.numeric(dynamicNetworkGraphicObject$terminu)
  #return(newedges)
}
...
FBNNetwork.Graph.StaticNetworkInSlice<-function(networkobject,timepoint,dynamicNetworkGraphicObject)
{
  ...
  lnodes <- data.frame(label = c("Gene", "Activate Function (+)","Inhibit Function (-)"),
                       shape = c("ellipse","box","box"), color = c("lightblue", "lightgreen","orange"),shadow=c(TRUE,FALSE,FALSE))

  # edges data.frame for legend
  ledges <- data.frame(color = c("darkblue", "darkred","grey","green","red"),
                       label = c("activate", "inhibit", "decay", "activated input","deactivated input"),arrows =c("to", "to","to","none","none"), dashes=c(FALSE,TRUE,TRUE,FALSE,TRUE),shadow=c(TRUE,TRUE,TRUE,FALSE,FALSE))



  visNetwork(newnodes, newedges, width="100%", height="100%", main=paste("Fundamental Boolean Networks in the time step of "," ",timepoint,sep="", collapse = ""))%>%
...
#' coming later
#' @export
FBNNetwork.Graph.StaticNetwork<-function(FBNnetwork)
{
 ...
 #visLegend(addEdges = ledges, addNodes = lnodes, useGroups = FALSE)%>%
  visNetwork(newnodes, newedges, width="100%", height="100%", main=paste("Fundamental Boolean Networks",sep="", collapse = ""))%>%
    visLegend(addEdges = ledges, addNodes = lnodes, useGroups = FALSE)%>%
    visInteraction(dragNodes = TRUE, dragView = TRUE, zoomView = TRUE)
  # & as.numeric(dynamicNetworkGraphicObject$onset)>=timepoint & timepoint<=as.numeric(dynamicNetworkGraphicObject$terminu)
  #return(newedges)
...
FBNNetwork.Graph.GenerateDynamicNetworkGraphicObject<-function(networkobject,fromtimepoint,totimepoint,dynamicNetworkGraphicObject,timeseries)
{
  ...
  while(index<totimepoint)
  {
    ...
    #fromfilterddynamic<-dynamicNetworkGraphicObject[which(as.numeric(dynamicNetworkGraphicObject$onset)<index & index<=as.numeric(dynamicNetworkGraphicObject$terminus)),]
    ...
    tonewedges[which(tonewedges$from%in%filterednodeFromGene$id),]$from=paste(tonewedges[which(tonewedges$from%in%filterednodeFromGene$id),]$from,"_",index,sep="")
    ...
    tonewedges[which(tonewedges$to%in%filterednodeToGene$id),]$to=paste(tonewedges[which(tonewedges$to%in%filterednodeToGene$id),]$to,"_",nextindex,sep="")
    ...
    tonewedges[which(tonewedges$from%in%filterednodeFromTF$id),]$from=paste(tonewedges[which(tonewedges$from%in%filterednodeFromTF$id),]$from,"_",nextindex,sep="")
  ...
  lnodes <- data.frame(label = c("Activated Gene", "Inhibited Gene", "Activate Function (+)","Inhibit Function (-)"),
                       shape = c("ellipse","ellipse","box","box"), color = c("lightblue","pink", "lightgreen","orange"),shadow=c(TRUE,TRUE,FALSE,FALSE))
  ...
  visNetwork(newnodes, newedges, width="100%", height="100%", main=paste("Dynamic Fundamental Boolean Networks from the time point of"," ",fromtimepoint," ", "to ",totimepoint,sep="", collapse = ""))%>%
    visLegend(addEdges = ledges, addNodes = lnodes, useGroups = FALSE)%>%
    visInteraction(dragNodes = TRUE, dragView = TRUE, zoomView = TRUE)
  # & as.numeric(dynamicNetworkGraphicObject$onset)>=timepoint & timepoint<=as.numeric(dynamicNetworkGraphicObject$terminu)
  #return(newedges)
...
FBNNetwork.Graph.DrawingAttractorInternal<-function(networkobject,dynamicNetworkGraphicObject,matrix)
{
  ...
  while(index<length(cNames))
  {
    ...
    #individualnode<-networkobject$nodes[which(!filterednodeFromGene$nodes$title%in%genes),]
    #individualnode$color="grey"
    ...
    if(length(independeGenes)>0)
    {
      for(k in seq_along(independeGenes))
      {
        ...
        {
          makeupnode<-list(id=paste(gene,"_",index,sep=""),shape="ellipse",color="pink",type="gene",value=4,label=paste(gene,"_",index,sep=""),shadow=TRUE,group="Gene",title=gene,level=startlevel)
        }else
        {
          makeupnode<-list(id=paste(gene,"_",index,sep=""),shape="ellipse",color="lightblue",type="gene",value=4,label=paste(gene,"_",index,sep=""),shadow=TRUE,group="Gene",title=gene,level=startlevel)
        }
    ...
    if(length(independeGenes)>0)
    {
      for(k in seq_along(independeGenes))
      {
        ...
        if(as.numeric(genestate==0))
        {
          makeupnode<-list(id=paste(gene,"_",nextindex,sep=""),shape="ellipse",color="pink",type="gene",value=4,label=paste(gene,"_",nextindex,sep=""),shadow=TRUE,group="Gene",title=gene,level=startlevel+2)
        }else
        {
          makeupnode<-list(id=paste(gene,"_",nextindex,sep=""),shape="ellipse",color="lightblue",type="gene",value=4,label=paste(gene,"_",nextindex,sep=""),shadow=TRUE,group="Gene",title=gene,level=startlevel+2)
        }
  ...
  visNetwork(newnodes, newedges, width="100%", height="100%", main=paste("Dynamic Fundamental Boolean Networks from the time point of"," ",1," ", "to ",length(cNames),sep="", collapse = ""))%>%
    visLegend(addEdges = ledges, addNodes = lnodes, useGroups = FALSE,width=0.1)%>%
    visInteraction(dragNodes = TRUE, dragView = TRUE, zoomView = TRUE)%>%visEdges(arrows = "from") %>%
...
#' coming later
#' @export
FBNNetwork.Graph.ConvertToNetworkGraphicObject<-function(FBNnetwork)
{
  if (!(inherits(FBNnetwork,"FundamentalBooleanNetwork")))
    stop("Network must be inherited from FundamentalBooleanNetwork")
  ...
  for (i in seq_along(FBNnetwork$interactions)) {
    ...
    for(j in seq_along(FBNnetwork$interactions[[i]]))
    {
      ...
      for(n in seq_along(FBNnetwork$interactions[[i]][[j]]$input))
      {
        ...
        splitedexpression<-splitExpression(FBNnetwork$interactions[[i]][[j]]$expression)
...
FBNNetwork.Graph<-function(fbnNetwork, type="static", timeseriesMatrix=NULL, fromTimePoint=1, toTimePoint=5,networkobject=NULL)
{
  ...
  switch(type,
         static=FBNNetwork.Graph.StaticNetwork(fbnNetwork),
